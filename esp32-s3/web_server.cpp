...
void handleLogin() {
  if (SERVER_HAS_ARG("username") && SERVER_HAS_ARG("password")) {
    if (SERVER_ARG("username") == WEB_AUTH_USER && SERVER_ARG("password") == WEB_AUTH_PASS) {
      ...
      tokenActive = true;
      // Set HttpOnly session cookie so the browser sends it on every page navigation,
      // allowing the server to gate HTML pages without exposing their contents.
...
void handleNotFound() {
  ...
  if (path == "/" || path.endsWith(".html")) {
    if (!validateSessionToken(getSessionCookie())) {
      // Serve a minimal login gate — just enough to load auth.js which shows the form
